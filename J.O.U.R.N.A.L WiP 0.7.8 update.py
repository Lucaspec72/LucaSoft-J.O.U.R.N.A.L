...
try:
    ...
    raise SystemExit
BUFFERSIZE = 64 * 1024 
DATAFOLDER = os.path.join(os.path.dirname(sys.argv[0]), "LucaSoft J.O.U.R.N.A.L\\")
WRITESPEED = 0.03
...
def writeEntryFile(entryFileName,user,pwd,entry):
    ...
    with open(f'{DATAFOLDER}{user}/{entryFileName}.entry', 'w', encoding="utf-8") as file:
...
class dialogue:
    ...
    def loginNewUsrPrompt():
        text=['The user "',f'{currentUser}','" does not currently exists, do you want to create it ? (press ','Y',' to confirm) ']
    ...
    def mainMenu():
        ...
        return text,style
    def createLog1():
        text=[f'Creating Log {entryName} (Log N°{len(folderFiles)+1})']
        style=['']
        return text,style
    def createLog2():
    ...
    def createLog3():
        ...
        return text,style
    def createLog4():
        text=['Please Enter Entry File Name : ']
        style=['']
        return text,style
    def listEntries1():
    ...
    def goBackToMain():
        ...
        return text,style
    def viewEntry1():
        text=['Select Which Entry to View : ']
        style=['']
        return text,style
    def viewEntry2():
    ...
    for root, dirs, files in os.walk(f"{DATAFOLDER}{currentUser}"):
        for file in files:
            if file.endswith(".entry"):
                filesArray.insert(len(filesArray),file)
...
def clear():
    cl()
    print(f"{colors.data}J.O.U.R.N.A.L      {colors.label}v{progVer}{colors.data}        (C) Copyright 2024 Lucaspec72\n{colors.label}")
...
while(True):
    ...
    clear()
    if(instruction == "1"):
        write(dialogue.createLog4())
        entryName = askUser()
        if(entryName == ""):
            if(len(folderFiles) > 0):
                entryName = int(folderFiles[-1].replace(".entry","")) +1
            else:
                entryName = 1
        ...
        if(logEntry != ""):
             ...
             writeEntryFile(entryName,currentUser,currentPassword,encrypt(f"{currentUser}'s Journal, Entry N°{len(folderFiles)+1}. Written on {month[(timeFormated.month-1)]} {timeFormated.day} {timeFormated.year} at {format(timeFormated.hour, '02')}:{format(timeFormated.minute, '02')}\n\n{logEntry}",currentPassword))
